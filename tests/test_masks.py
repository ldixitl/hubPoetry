...
def test_mask_card_number(numbers: str, expected: str) -> None:
    assert get_mask_card_number(numbers) == expected


@pytest.mark.parametrize("numbers_invalid_type", [None, 1234567898765432, []])
...
def test_get_mask_card_number_invalid_value(numbers_invalid_value: str) -> None:
...
def test_mask_account_number(numbers: str, expected: str) -> None:
    assert get_mask_account(numbers) == expected


@pytest.mark.parametrize("numbers_invalid_type", [None, 11112222333344445555, []])
def test_get_mask_account_invalid_type(numbers_invalid_type: Union[None, int, List]) -> None:
    with pytest.raises(TypeError):
...
def test_get_mask_account_invalid_value(numbers_invalid_value: str) -> None:
    with pytest.raises(ValueError):
